...
import { ChevronLeft, ChevronRight, Snowflake, Flame, Package, X } from 'lucide-react';
...
// Extract HeaderFilter outside to prevent re-mounting and focus loss on every render
...
const HeaderFilter: React.FC<HeaderFilterProps> = ({ 
  ...
  <div className={`flex flex-col gap-2 ${width || 'w-full'}`}>
...
export const DataTable: React.FC<DataTableProps> = ({ rows }) => {
  ...
  const [filters, setFilters] = useState({
    taille_conteneur: '', // Added size filter
    num_conteneur: '',
    code_iso: '',
    categorie: '',
    statut: '',
  ...
  const filteredRows = useMemo(() => {
    return rows.filter(row => {
      ...
      const matches = (dataVal: string | number | undefined, filterVal: string) => {
        if (!filterVal) return true;
        const stringVal = (dataVal || '').toString().toLowerCase();
        return stringVal.includes(filterVal.toLowerCase().trim());
      };

      return (
        matches(row.taille_conteneur, filters.taille_conteneur) &&
          ...
          <thead className="bg-slate-50">
            <tr>
              <th className="px-6 py-3 text-left w-24 align-top">
                 <HeaderFilter 
                    label="TAILLE" 
                    value={filters.taille_conteneur}
                    onChange={(val) => handleFilterChange('taille_conteneur', val)}
                    placeholder="20/40"
                 />
              </th>
              ...
              <th className="px-6 py-3 text-left w-48 align-top">
                <HeaderFilter 
                    label="N° CONTENEUR" 
                    value={filters.num_conteneur}
                    onChange={(val) => handleFilterChange('num_conteneur', val)}
                />
              </th>
              ...
              <th className="px-6 py-3 text-left w-28 align-top">
                <HeaderFilter 
                    label="TYPE ISO" 
                    value={filters.code_iso}
                    onChange={(val) => handleFilterChange('code_iso', val)}
                    placeholder="Ex: 22G1" 
                />
              </th>
              ...
              <th className="px-6 py-3 text-left w-32 align-top">
                <HeaderFilter 
                    ...
                    onChange={(val) => handleFilterChange('statut', val)}
                    placeholder="FCL/LCL" 
                />
              </th>
              ...
              <th className="px-6 py-3 text-left w-40 align-top">
                <HeaderFilter 
                    label="RÉF BL" 
                    value={filters.num_bl}
                    onChange={(val) => handleFilterChange('num_bl', val)}
                />
              </th>
              ...
              <th className="px-6 py-3 text-left min-w-[200px] align-top">
                <HeaderFilter 
                    ...
                    value={filters.client_final}
                    onChange={(val) => handleFilterChange('client_final', val)}
                />
              </th>
            </tr>
          </thead>
